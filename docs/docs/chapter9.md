---
sidebar_position: 10
title: "Chapter 9: Serving Data for Analytics, ML, and Reverse ETL"
description: "Learn how to serve data for analytics, machine learning, and reverse ETL - the final stage of the data engineering lifecycle"
---
...
1. [General Considerations for Serving Data](#general-considerations-for-serving-data)
   - Trust
...
4. [Ways to Serve Data for Analytics and ML](#ways-to-serve-data-for-analytics-and-ml)
   - File Exchange
   - Databases
   - Streaming Systems
   - Query Federation
...
5. [Reverse ETL](#reverse-etl)
6. [Whom You'll Work With](#whom-youll-work-with)
7. [Undercurrents](#undercurrents)

## General Considerations for Serving Data

**In plain English:** Before serving data, ensure people trust it, understand who will use it and why, and establish clear definitions of what the data means.

**In technical terms:** Serving data requires establishing data quality processes, understanding use cases and user personas, implementing proper access controls, and creating semantic layers for consistent business logic.

**Why it matters:** Without trust and clear definitions, even the most sophisticated data architecture will fail to deliver value. Stakeholders won't use data they don't believe in, and inconsistent definitions lead to conflicting insights.
...
> **Insight**
>
> Trust is everything in data engineering. It takes a long time to earn, and it's easy to lose. Once trust is gone, earning it back is insanely difficult.
...
**In plain English:** Like a chef who needs to know who they're cooking for and what occasion, data engineers need to understand what actions the data will trigger and who will take those actions.

**In technical terms:** Serving requirements vary dramatically based on use case (reporting, real-time decisioning, ML training) and user persona (executive, analyst, data scientist, automated system).

**Why it matters:** Building data products without understanding the use case is like building a house without knowing what it's for. You might create something nobody wants or needs, wasting time and resources.
...
When creating a data product, it's useful to think of the "jobs to be done." A user "hires" a product for a "job to be done." This means you need to know what the user wants—i.e., their motivation for "hiring" your product. A classic engineering mistake is simply building without understanding the requirements, needs of the end user, or product/market fit. This disaster happens when you build data products nobody wants to use.

> **Insight**
>
> A good data product has positive feedback loops. More usage generates more useful data, which is used to improve the data product. Rinse and repeat.
...
**In plain English:** Self-service data is like a buffet versus a chef's table—sometimes people want to serve themselves, but often they just want the perfect dish delivered to them.

**In technical terms:** Self-service data products enable users to build their own reports and analyses without technical assistance, but successful implementation requires the right audience, proper guardrails, and realistic expectations.

**Why it matters:** Many self-service initiatives fail because they try to be everything to everyone. Understanding when self-service makes sense saves resources and improves user satisfaction.
...
**In plain English:** Everyone in your organization needs to speak the same language about data—what "customer" means, how revenue is calculated, and which version of truth to use.

**In technical terms:** Data definitions specify the meaning of entities and attributes, while data logic encodes the formulas and rules for deriving metrics and calculations consistently across the organization.

**Why it matters:** When different departments calculate the same metric differently, you get conflicting insights and eroded trust. Consistent definitions are foundational to reliable analytics.
...
**In plain English:** Instead of a centralized data team serving everyone, each business domain takes responsibility for their own data and shares it with others.

**In technical terms:** Data mesh implements domain-oriented decentralized data ownership with federated governance, treating data as a product that domains both produce for others and consume from others.

**Why it matters:** Data mesh shifts serving from a centralized bottleneck to a distributed responsibility model, enabling scale and agility while maintaining quality standards.
...
**In plain English:** Analytics is like detective work—discovering patterns, trends, and insights hidden in data to help people make better decisions.

**In technical terms:** Analytics encompasses discovering, exploring, identifying, and making visible key insights and patterns within data using statistical methods, reporting, and BI tools.

**Why it matters:** Analytics transforms raw data into actionable insights that drive business strategy, operational improvements, and competitive advantage.
...
**In plain English:** Business analytics is like looking in the rearview mirror and at the road ahead to decide where to steer your company.

**In technical terms:** Business analytics uses historical and current data with statistical methods and domain expertise to inform strategic and actionable business decisions.

**Why it matters:** Strategic decisions backed by data analysis lead to better outcomes than gut feelings alone, helping companies allocate resources effectively and compete successfully.
...
**In plain English:** If business analytics is looking in the rearview mirror, operational analytics is watching your dashboard in real time and hitting the brakes when danger appears.

**In technical terms:** Operational analytics uses real-time or near-real-time data to enable immediate action and automated responses to current conditions.

**Why it matters:** When seconds matter—detecting fraud, preventing system failures, or responding to customer behavior—operational analytics can mean the difference between success and disaster.
...
An example of operational analytics is real-time application monitoring. Many software engineering teams want to know how their application is performing; if issues arise, they want to be notified immediately. The engineering team might have a dashboard that shows the key metrics such as requests per second, database I/O, or whatever metrics are important. Certain conditions can trigger scaling events, adding more capacity if servers are overloaded. If certain thresholds are breached, the monitoring system might also send alerts via text message, group chat, and email.

#### Business and Operational Analytics Convergence

> **Insight**
>
> The line between business and operational analytics has begun to blur. As streaming and low-latency data become more pervasive, it is only natural to apply operational approaches to business analytics problems.
...
**In plain English:** Embedded analytics is like putting a dashboard directly into your customer's hands—giving them insights about their own usage and behavior right in your app.

**In technical terms:** Embedded analytics provides end-user-facing dashboards and metrics within applications, enabling data-driven features and personalization for external users.

**Why it matters:** Modern customers expect data-driven experiences. Embedded analytics turns your application into a data product that provides value through insights, not just functionality.
...
<ComparisonTable
  title="Embedded Analytics Requirements"
  headers={["Requirement", "Challenge", "Solution Approach"]}
  rows={[
    ["Low Data Latency", "Users expect near real-time updates", "Stream processing, CDC, micro-batches"],
    ["Fast Query Performance", "Results needed in seconds", "Pre-aggregation, caching, columnar storage"],
    ["High Concurrency", "Many users querying simultaneously", "Distributed architecture, query optimization"]
  ]}
/>
...
**In plain English:** Data engineers are the pit crew for data scientists—they don't build the race car (the model), but they ensure it has high-quality fuel (data) to run.

**In technical terms:** Data engineers handle data acquisition, transformation, and featurization pipelines that enable data scientists to train, validate, and deploy ML models effectively.

**Why it matters:** ML models are only as good as the data they're trained on. Poor data quality or improper preparation makes even the most sophisticated algorithms useless.
...
- All data that is used for ML is converted to numbers. Ensure that the data can be properly converted during the feature-engineering process
...
- How does the data engineering lifecycle intersect with the ML lifecycle at your company?
- Know when it's appropriate to train locally, on a cluster, or at the edge. When would you use a GPU over a CPU?
...
- The use of structured versus unstructured data

> **Insight**
>
> ML is a vast subject area. If you'd like to learn more about ML, we suggest reading Hands on Machine Learning with Scikit-Learn, Keras, and TensorFlow by Aurélien Géron (O'Reilly).

## Ways to Serve Data for Analytics and ML

**In plain English:** There are many ways to get data to users—from simple file sharing to sophisticated streaming systems—each with its own trade-offs.

**In technical terms:** Data serving methods include file exchange, databases (OLAP/OLTP), streaming systems, query federation, data sharing platforms, semantic layers, and notebooks, each optimized for different use cases.

**Why it matters:** Choosing the right serving method impacts performance, cost, security, and user experience. One size does not fit all.
...
**In plain English:** Databases are like well-organized libraries where data is carefully cataloged, secured, and ready to be queried by anyone with the right permissions.

**In technical terms:** OLAP databases (data warehouses and data lakes) provide structured, performant access to large datasets with fine-grained security controls and support for concurrent queries.

**Why it matters:** Databases impose structure and governance on data while delivering high performance for analytical workloads, making them the backbone of most serving architectures.

Databases are a critical layer in serving data for analytics and ML. For this discussion, we'll implicitly keep our focus on serving data from OLAP databases (e.g., data warehouses and data lakes). In the previous chapter, you learned about querying databases. Serving data involves querying a database and then consuming those results for a use case. An analyst or data scientist might query a database by using a SQL editor and export those results to a CSV file for consumption by a downstream application, or analyze the results in a notebook.
...
**In plain English:** Query federation is like asking one librarian who can search multiple libraries at once, rather than visiting each library yourself.

**In technical terms:** Federation pulls data from multiple heterogeneous sources (data lakes, RDBMSs, data warehouses) using distributed query virtualization engines without centralizing the data.

**Why it matters:** Federation enables flexible exploratory analysis and maintains tight access control without the overhead of setting up ETL pipelines for every data source.
...
<DiagramContainer title="A federated query with three data sources">
  <Row>
    <Box color={colors.blue}>OLTP Database</Box>
    <Box color={colors.blue}>Data Lake</Box>
    <Box color={colors.blue}>OLAP Warehouse</Box>
  </Row>
  <Row>
    <Column span={3}>
      <Arrow direction="up" />
      <Box color={colors.green}>Federated Query Engine</Box>
      <Arrow direction="up" />
      <Box color={colors.purple}>Analytics User</Box>
    </Column>
  </Row>
</DiagramContainer>

When serving data for federated queries, you should be aware that the end user might be querying several systems—OLTP, OLAP, APIs, filesystems, etc. Instead of serving data from a single system, you're now serving data from multiple systems, each with its usage patterns, quirks, and nuances. This poses challenges for serving data. If federated queries touch live production source systems, you must ensure that the federated query won't consume excessive resources in the source.
...
**In plain English:** A semantic layer is like having a universal translator that ensures everyone speaks the same data language and calculates metrics the same way.

**In technical terms:** Semantic and metrics layers maintain business logic, metric definitions, and data transformations in a centralized, reusable layer that generates consistent queries across all downstream uses.

**Why it matters:** Without a semantic layer, every analyst writes their own version of "revenue" or "active users," leading to conflicting numbers and eroded trust in data.
...
> **Insight**
>
> Metrics layer tools help solve a central question in analytics that has plagued organizations since people have analyzed data: "Are these numbers correct?"
...
**In plain English:** Notebooks are like digital lab notebooks where data scientists experiment, document their work, and share findings all in one interactive document.

**In technical terms:** Notebooks (primarily Jupyter) provide interactive computational environments that combine code execution, visualization, and documentation, connecting to various data sources programmatically.

**Why it matters:** Notebooks are the primary workspace for data scientists, so understanding how to serve data to them efficiently is crucial for enabling productive data science work.
...
> **Insight**
>
> Incorrectly handled credentials in notebooks and data science code are a major security risk. Credentials should never be embedded in code; ideally, data scientists use credential managers or CLI tools to manage access.
...
**In plain English:** Reverse ETL is like taking insights from your data warehouse and pumping them back into the operational systems where people actually work.

**In technical terms:** Reverse ETL loads processed data from OLAP systems back into source systems (CRMs, ad platforms, operational databases), completing a bidirectional data flow.

**Why it matters:** Data insights are only valuable if they drive action. Reverse ETL puts insights directly into the tools people use daily, reducing friction and increasing data-driven actions.
...
Using reverse ETL and loading the scored leads back into the CRM is the easiest and best approach for this data product. Reverse ETL takes processed data from the output side of the data engineering lifecycle and feeds it back into source systems.

<DiagramContainer title="Reverse ETL">
  <ProcessFlow>
    <Box color={colors.blue}>Source System (CRM)</Box>
    <Arrow label="Extract" />
    <Box color={colors.green}>Data Warehouse</Box>
    <Arrow label="Transform & Analyze" />
    <Box color={colors.green}>ML Model / Analytics</Box>
    <Arrow label="Reverse ETL" />
    <Box color={colors.purple}>Source System (CRM)</Box>
  </ProcessFlow>
</DiagramContainer>

> **Note**
>
> Instead of reverse ETL, we, the authors, half-jokingly call it bidirectional load and transform (BLT). The term reverse ETL doesn't quite accurately describe what's happening in this process. Regardless, the term has stuck in the popular imagination and press, so we'll use it throughout the book.
...
> **Insight**
>
> Reverse ETL inherently creates feedback loops. Be careful, and build in monitoring and guardrails to prevent runaway processes.
...
> **Insight**
>
> "Data is a silent killer." The undercurrents come to a head in the serving stage. Serving is your final chance to make sure your data is in great shape before it gets into the hands of end users.
...
<DiagramContainer title="Build, learn, improve">
  <ProcessFlow direction="circular">
    <Box color={colors.green}>Build</Box>
    <Arrow />
    <Box color={colors.blue}>Learn</Box>
    <Arrow />
    <Box color={colors.purple}>Improve</Box>
    <Arrow />
  </ProcessFlow>
</DiagramContainer>
...
**Previous:** [Chapter 8: Queries, Modeling, and Transformation](./chapter8.md) | **Next:** [Chapter 10: Security and Privacy](./chapter10.md)
