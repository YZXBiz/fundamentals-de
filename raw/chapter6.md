...
On the one hand, these disks have seen extraordinary improvements in performance, storage density, and cost.1 On the other hand, SSDs dramatically outperform magnetic disks on various metrics. Currently, commercial magnetic disk drives cost roughly 3 cents per gigabyte of capacity. (Note that we’ll frequently use the abbreviations HDD and SSD to denote rotating magnetic disk and solid-state drives, respectively.)
...
Write the returned metadata for the object version to the strongly consistent database.
...
Fetch the latest object metadata from the strongly consistent database.

Query object metadata using the object key. Read the object data if it matches the metadata fetched from the consistent database.

If the object metadata does not match, repeat step 2 until the latest version of the object is returned.
...
4 Valliappa Lakshmanan and Jordan Tigani, Google BigQuery: The Definitive Guide (Sebastopol, CA: O’Reilly, 2019), 16–17, 188, https://oreil.ly/5aXXu.

table of contents
search
Settings
Previous chapter 5. Data Generation in Source Systems
Next chapter 7. Ingestion
